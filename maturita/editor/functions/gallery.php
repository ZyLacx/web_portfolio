...
echo "</div><div class='content-div'>";
    ...
    $query = "SELECT max(id) FROM gallery_images";
	...
	echo "<a class='gallery-back' href='?'>Späť</a>";
        $query = "SELECT * FROM gallery_images WHERE id='{$_GET["id"]}'";
        $result = mysqli_query($conn, $query);
            ...
            foreach($img_array as $i => $img){
                echo "<div class='item no-drag' id='$i'>
                        <div class='item-content'>
                            <img id='$img' src='../files/img/thumbnails/$img'>
                        </div>
                    </div>";
            }
            echo "</div>";
        ...
        $query = "SELECT * FROM gallery ORDER BY muuri_index";
        $result = mysqli_query($conn, $query);
        if(mysqli_num_rows($result) > 0){
            $dbData = [];
